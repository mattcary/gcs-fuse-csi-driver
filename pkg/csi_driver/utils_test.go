...
func TestJoinMountOptions(t *testing.T) {
	testCases := []struct {
		...
		{
			name:            "should add new local socket",
			existingOptions: []string{},
			newOptions:      []string{"experimental-local-socket-address=128.0.0.1"},
			expectedOptions: []string{"experimental-local-socket-address=128.0.0.1"},
		},
		{
			name:            "should keep existing local socket",
			existingOptions: []string{"experimental-local-socket-address=128.0.0.1"},
...
func TestIsSidecarVersionSupportedForGivenFeature(t *testing.T) {
	...
	t.Run("checking if sidecar version is supported for given version and feature", func(t *testing.T) {
		...
		for _, tc := range testCases {
			...
			actual := d.isSidecarVersionSupportedForGivenFeature(tc.imageName, tc.minFeatureVersionSupported)
...
func TestParseVolumeAttributes(t *testing.T) {
	testCases := []struct {
		...
		expectMultiNICIndex                   bool
		expectedMultiNICIndex                 int
	}{
		{
		...
		{
			name:                  "value set for VolumeContextKeyMultiNICIndex",
			volumeContext:         map[string]string{VolumeContextKeyMultiNICIndex: "1"},
			expectedErr:           false,
			expectedMountOptions:  []string{},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			args, err := parseVolumeAttributes([]string{}, tc.volumeContext)
			...
			if tc.expectedSkipBucketAccessCheck != args.skipCSIBucketAccessCheck {
				t.Errorf("Got skipBucketAccessCheck %v, but expected %v", args.skipCSIBucketAccessCheck, tc.expectedSkipBucketAccessCheck)
			}
			if tc.expectedDisableMetricsCollection != args.disableMetricsCollection {
				t.Errorf("Got disableMetricsCollection %v, but expected %v", args.disableMetricsCollection, tc.expectedDisableMetricsCollection)
			}
			if tc.expectedEnableCloudProfilerForSidecar != args.enableCloudProfilerForSidecar {
				t.Errorf("Got enableCloudProfilerForSidecar %v, but expected %v", args.enableCloudProfilerForSidecar, tc.expectedEnableCloudProfilerForSidecar)
			}

			less := func(a, b string) bool { return a > b }
			if diff := cmp.Diff(args.fuseMountOptions, tc.expectedMountOptions, cmpopts.SortSlices(less)); diff != "" {
				t.Errorf("unexpected options args (-got, +want)\n%s", diff)
			}
